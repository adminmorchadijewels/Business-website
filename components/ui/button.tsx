...
function Button({
  ...
  return (
    <ButtonPrimitive
      data-slot="button"
      render={render}
      // When `render` swaps the element for a non-button (e.g. a Next.js
      // <Link> → <a>), Base UI must be told it's not a native <button>,
      // otherwise it warns. Default accordingly; callers can still override
      // (pass nativeButton when rendering a real <button> via `render`).
